...
exports._Math = {
    ...
    findAngle: function (b0, b1) {
        ...
        var a = b0.end.minus(b1.end).lengthSq(), b = b0.end.minus(b0.start).lengthSq(), c = b1.end.minus(b0.start).lengthSq();
        var angle;
        var r = (a + b - c) / Math.sqrt(4 * a * b);
        if (r <= -1)
            angle = Math.PI;
        else if (r >= 1)
            angle = 0;
            ...
            angle = Math.acos(r);
        // sign
        var sign = b0.end.x * b1.end.y - b0.end.y * b1.end.x;
    ...
    perpendicular: function (a, b) {
        return exports._Math.nearEquals(a.dot(b), 0.0, 0.01) ? true : false;
    ...
    genPerpendicularVectorFrisvad: function (v) {
        var nv = v.clone();
        if (v.z < -0.9999999)
            return nv.set(0, -1, 0); // Handle the singularity
        var a = 1 / (1 + v.z);
        return nv.set(1 - v.x * v.x * a, -v.x * v.y * a, -v.x).normalize();
    },
    // rotation
    rotateXDegs: function (v, angle) {
        return v.clone().rotate(angle * exports._Math.toRad, 'X');
    },
    rotateYDegs: function (v, angle) {
        return v.clone().rotate(angle * exports._Math.toRad, 'Y');
    },
    rotateZDegs: function (v, angle) {
        return v.clone().rotate(angle * exports._Math.toRad, 'Z');
    },
    // distance
    ...
    distanceBetween: function (v1, v2) {
        var dx = v2.x - v1.x;
        var dy = v2.y - v1.y;
        var dz = v1 instanceof V3_1.V3 && v2 instanceof V3_1.V3 ? v2.z - v1.z : 0;
        return Math.sqrt(dx * dx + dy * dy + dz * dz);
    },
    // ______________________________ 2D _____________________________
    rotateDegs: function (v, angle) {
        return v.clone().rotate(angle * exports._Math.toRad);
    },
    validateDirectionUV: function (directionUV) {
        if (directionUV.length() < 0)
            Tools_1.Tools.error('vector direction unit vector cannot be zero.');
    },
    validateLength: function (length) {
        if (length < 0)
            Tools_1.Tools.error('Length must be a greater than or equal to zero.');
    },
};
