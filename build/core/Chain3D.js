...
var Chain3D = /** @class */ (function () {
    function Chain3D() {
        ...
        this.chainLength = 0;
        this.numBones = 0;
        this.baseLocation = new V3_1.V3();
        this.fixedBaseMode = true;
        this.baseboneConstraintType = constants_1.BaseboneConstraintType.NONE;
        this.baseboneConstraintUV = new V3_1.V3();
        this.baseboneRelativeConstraintUV = new V3_1.V3();
        this.baseboneRelativeReferenceConstraintUV = new V3_1.V3();
        this.lastTargetLocation = new V3_1.V3(constants_1.MAX_VALUE, constants_1.MAX_VALUE, constants_1.MAX_VALUE);
        this.lastBaseLocation = new V3_1.V3(constants_1.MAX_VALUE, constants_1.MAX_VALUE, constants_1.MAX_VALUE);
        ...
        this.embeddedTarget = new V3_1.V3();
        this.useEmbeddedTarget = false;
    }
    Chain3D.prototype.clone = function () {
        var c = new Chain3D();
    ...
    Chain3D.prototype.clear = function () {
        var i = this.numBones;
        while (i--)
            this.removeBone(i);
        this.numBones = 0;
    ...
    Chain3D.prototype.removeBone = function (id) {
        if (id < this.numBones) {
    ...
    Chain3D.prototype.addConsecutiveBone = function (directionUV, length) {
        if (this.numBones > 0) {
            ...
            this.addBone(new Bone3D_1.Bone3D(this.bones[this.numBones - 1].end, undefined, directionUV.normalised(), length));
        }
    };
    Chain3D.prototype.addConsecutiveFreelyRotatingHingedBone = function (directionUV, length, type, hingeRotationAxis) {
        this.addConsecutiveHingedBone(directionUV, length, type, hingeRotationAxis, 180, 180, Math_1._Math.genPerpendicularVectorQuick(hingeRotationAxis));
    };
    Chain3D.prototype.addConsecutiveHingedBone = function (DirectionUV, length, type, HingeRotationAxis, clockwiseDegs, anticlockwiseDegs, hingeReferenceAxis) {
    ...
    Chain3D.prototype.addConsecutiveRotorConstrainedBone = function (boneDirectionUV, length, constraintAngleDegs) {
        if (this.numBones === 0)
            return;
        ...
        var bone = new Bone3D_1.Bone3D(this.bones[this.numBones - 1].end, undefined, boneDirectionUV, length);
    ...
    //      GET
    // -------------------------------
    Chain3D.prototype.getBoneConnectionPoint = function () {
        return this.boneConnectionPoint;
    };
    Chain3D.prototype.getConnectedBoneNumber = function () {
        return this.connectedBoneNumber;
    };
    Chain3D.prototype.getConnectedChainNumber = function () {
        return this.connectedChainNumber;
    };
    Chain3D.prototype.getBaseboneConstraintType = function () {
        return this.baseboneConstraintType;
    };
    Chain3D.prototype.getBaseboneConstraintUV = function () {
        if (!(this.baseboneConstraintType === constants_1.BaseboneConstraintType.NONE))
            return this.baseboneConstraintUV;
    };
    Chain3D.prototype.getBaseLocation = function () {
        return this.bones[0].start;
    };
    Chain3D.prototype.getEffectorLocation = function () {
        return this.bones[this.numBones - 1].end;
    };
    Chain3D.prototype.getLastTargetLocation = function () {
        return this.lastTargetLocation;
    };
    Chain3D.prototype.getLiveChainLength = function () {
        var lng = 0;
        var i = this.numBones;
        while (i--)
    ...
    Chain3D.prototype.getBaseboneRelativeReferenceConstraintUV = function () {
        return this.baseboneRelativeReferenceConstraintUV;
    };
    // -------------------------------
    //      SET
    // -------------------------------
    Chain3D.prototype.setConnectedBoneNumber = function (boneNumber) {
        this.connectedBoneNumber = boneNumber;
    };
    Chain3D.prototype.setConnectedChainNumber = function (chainNumber) {
    ...
    Chain3D.prototype.setBaseboneRelativeReferenceConstraintUV = function (uv) {
        this.baseboneRelativeReferenceConstraintUV = uv.normalised();
    };
    Chain3D.prototype.setBaseboneConstraintUV = function (uv) {
        this.baseboneConstraintUV = uv.normalised();
    };
    Chain3D.prototype.setRotorBaseboneConstraint = function (type, constraintAxis, angleDegs) {
        // Sanity checking
        if (this.numBones === 0) {
            Tools_1.Tools.error('Chain must contain a basebone before we can specify the basebone constraint type.');
            return;
        }
        if (!(constraintAxis.length() > 0)) {
            Tools_1.Tools.error('Constraint axis cannot be zero.');
            return;
        }
        type = type || 'global';
        // Set the constraint type, axis and angle
        this.baseboneConstraintType = type === 'global' ? constants_1.BaseboneConstraintType.GLOBAL_ROTOR : constants_1.BaseboneConstraintType.LOCAL_ROTOR;
    ...
    Chain3D.prototype.setHingeBaseboneConstraint = function (type, hingeRotationAxis, cwDegs, acwDegs, hingeReferenceAxis) {
        // Sanity checking
        if (this.numBones === 0) {
            Tools_1.Tools.error('Chain must contain a basebone before we can specify the basebone constraint type.');
            return;
        ...
        if (hingeRotationAxis.length() <= 0) {
            Tools_1.Tools.error('Hinge rotation axis cannot be zero.');
            return;
        }
        if (hingeReferenceAxis.length() <= 0) {
            Tools_1.Tools.error('Hinge reference axis cannot be zero.');
            return;
        }
        if (!(Math_1._Math.perpendicular(hingeRotationAxis, hingeReferenceAxis))) {
            Tools_1.Tools.error('The hinge reference axis must be in the plane of the hinge rotation axis, that is, they must be perpendicular.');
            return;
        }
        type = type || 'global';
        // Set the constraint type, axis and angle
        this.baseboneConstraintType = type === 'global' ? constants_1.BaseboneConstraintType.GLOBAL_HINGE : constants_1.BaseboneConstraintType.LOCAL_HINGE;
        this.baseboneConstraintUV = hingeRotationAxis.normalised();
        this.bones[0].joint.setHinge(type === 'global' ? constants_1.JointType.J_GLOBAL : constants_1.JointType.J_LOCAL, hingeRotationAxis, cwDegs, acwDegs, hingeReferenceAxis);
    };
    Chain3D.prototype.setFreelyRotatingGlobalHingedBasebone = function (hingeRotationAxis) {
        this.setHingeBaseboneConstraint('global', hingeRotationAxis, 180, 180, Math_1._Math.genPerpendicularVectorQuick(hingeRotationAxis));
    };
    Chain3D.prototype.setGlobalHingedBasebone = function (hingeRotationAxis, cwDegs, acwDegs, hingeReferenceAxis) {
        this.setHingeBaseboneConstraint('global', hingeRotationAxis, cwDegs, acwDegs, hingeReferenceAxis);
    };
    Chain3D.prototype.setFreelyRotatingLocalHingedBasebone = function (hingeRotationAxis) {
        this.setHingeBaseboneConstraint('local', hingeRotationAxis, 180, 180, Math_1._Math.genPerpendicularVectorQuick(hingeRotationAxis));
    };
    Chain3D.prototype.setLocalHingedBasebone = function (hingeRotationAxis, cwDegs, acwDegs, hingeReferenceAxis) {
        this.setHingeBaseboneConstraint('local', hingeRotationAxis, cwDegs, acwDegs, hingeReferenceAxis);
    };
    Chain3D.prototype.setBaseLocation = function (baseLocation) {
        this.baseLocation.copy(baseLocation);
    };
    Chain3D.prototype.setFixedBaseMode = function (value) {
        ...
        if (!value && this.connectedChainNumber !== -1)
            return;
        if (this.baseboneConstraintType === constants_1.BaseboneConstraintType.GLOBAL_ROTOR && !value)
            return;
        // Above conditions met? Set the fixedBaseMode
        this.fixedBaseMode = value;
    };
    Chain3D.prototype.setMaxIterationAttempts = function (maxIterations) {
        if (maxIterations < 1)
            return;
        this.maxIteration = maxIterations;
    };
    Chain3D.prototype.setMinIterationChange = function (minIterationChange) {
        if (minIterationChange < 0)
            return;
        this.minIterationChange = minIterationChange;
    };
    Chain3D.prototype.setSolveDistanceThreshold = function (solveDistance) {
        if (solveDistance < 0)
            return;
    ...
    //
    // -------------------------------
    Chain3D.prototype.solveForEmbeddedTarget = function () {
        if (this.useEmbeddedTarget)
            return this.solveForTarget(this.embeddedTarget);
    };
    Chain3D.prototype.resetTarget = function () {
        this.lastBaseLocation = new V3_1.V3(constants_1.MAX_VALUE, constants_1.MAX_VALUE, constants_1.MAX_VALUE);
        this.currentSolveDistance = constants_1.MAX_VALUE;
    };
    ...
    Chain3D.prototype.solveForTarget = function (t) {
    ...
    Chain3D.prototype.solveIK = function (target) {
        if (this.numBones === 0)
            return;
        ...
        for (i = 0; i < this.numBones; i++) {
            ...
            if (i !== 0) {
                ...
                var hingeRotationAxis_1 = void 0, hingeReferenceAxis = void 0;
                switch (jointType) {
                    case constants_1.JointType.J_BALL:
                    ...
                    case constants_1.JointType.J_GLOBAL:
                        ...
                        hingeRotationAxis_1 = joint.getHingeRotationAxis();
                        hingeReferenceAxis = joint.getHingeReferenceAxis();
                        boneInnerToOuterUV.projectOnPlane(hingeRotationAxis_1);
                        // Constrain rotation about reference axis if required
                        if (!joint.freeHinge)
                            boneInnerToOuterUV.constrainedUV(hingeReferenceAxis, hingeRotationAxis_1, tmpMtx, joint.min, joint.max);
                        break;
                    case constants_1.JointType.J_LOCAL:
                        ...
                        hingeRotationAxis_1 = joint.getHingeRotationAxis().clone().applyM3(tmpMtx);
                ...
                var boneInnerToOuterUV = bone.getDirectionUV();
                var hingeRotationAxis_2 = void 0, hingeReferenceAxis = void 0;
                switch (this.baseboneConstraintType) {
                    case constants_1.BaseboneConstraintType.NONE: // Nothing to do because there's no basebone constraint
                        break;
                    case constants_1.BaseboneConstraintType.GLOBAL_ROTOR:
                    ...
                    case constants_1.BaseboneConstraintType.LOCAL_ROTOR:
                        ...
                        break;
                    case constants_1.BaseboneConstraintType.GLOBAL_HINGE:
                        hingeRotationAxis_2 = joint.getHingeRotationAxis();
                        hingeReferenceAxis = joint.getHingeReferenceAxis();
                        // Get the inner-to-outer direction of this bone and project it onto the global hinge rotation axis
                        boneInnerToOuterUV.projectOnPlane(hingeRotationAxis_2);
                        // Constrain as necessary
                        if (!joint.freeHinge)
                            boneInnerToOuterUV.constrainedUV(hingeReferenceAxis, hingeRotationAxis_2, tmpMtx, joint.min, joint.max);
                        break;
                    case constants_1.BaseboneConstraintType.LOCAL_HINGE:
                        hingeRotationAxis_2 = this.baseboneRelativeConstraintUV;
    ...
    Chain3D.prototype.updateChainLength = function () {
    ...
    Chain3D.prototype.cloneBones = function () {
